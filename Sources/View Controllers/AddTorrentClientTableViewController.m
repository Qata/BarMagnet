...
- (void)save {
  if (self.hostnameField.text.length && self.nameField.text.length) {
    ...
    NSDictionary *object = @{
      @"name" : [[NSOption fromNil:self.nameField.text] orSome:@""],
    ...
    if (!containsName) {
      ...
      [[UIAlertView.alloc initWithTitle:@"Error"
                                message:[NSString stringWithFormat:@"You already have a client named %@", self.nameField.text]
                               delegate:nil
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
  if (indexPath.section) {
    [[UIAlertView.alloc initWithTitle:@"SeedStuff.ca"
                              message:@"Seedstuff is a seedbox company who strives to be able to provide the best customer service and most reliable seedboxes "
                                      @"at an affordable price while keeping a commitment to ensuring their customers personal privacy. "
                             delegate:self

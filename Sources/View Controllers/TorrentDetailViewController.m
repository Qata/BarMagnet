...
- (void)viewDidLoad {
  ...
  self.identifierArray = @[
    @[ @"", @"Size", @"Downloaded", @"Uploaded", @"Completed", @"Date Added", @"Date Finished" ],
    @[ @"Download", @"Upload", @"Seeds Connected", @"Peers Connected", @"Ratio", @"ETA" ]
  ];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  case 0:
    ...
    case 4: {
      ...
      cell.detailTextLabel.text = [NSString stringWithFormat:@"%.1f%%", completeValue ? [hashDict[@"progress"] doubleValue] / completeValue * 100
                                                                                      : [hashDict[@"progress"] doubleValue] / [hashDict[@"size"] doubleValue]];
      break;
    }
    case 5:
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
  switch (section) {
  case 0:
    return [self.identifierArray[section] count] - 2 + self.client.supportsAddedDate +
           (self.client.supportsCompletedDate && [hashDict[@"progress"] doubleValue] == [self.client.class completeNumber].doubleValue);
    break;
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
  if (buttonIndex != [actionSheet cancelButtonIndex]) {
    [TorrentDelegate.sharedInstance.currentlySelectedClient addTemporaryDeletedJob:4 forKey:self.hashString];

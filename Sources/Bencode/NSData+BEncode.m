...
- (NSString *)magnetLink {
    ...
    if (!error) {
                ...
                for (NSArray *tracker in torrentArgsDict[@"announce-list"]) {
                    if ([tracker isKindOfClass:NSArray.class] && [tracker count]) {
                        for (NSString *trackerString in tracker) {
                            [trackers addObject:[NSString stringWithFormat:@"tr=%@", [[trackerString stringByReplacingOccurrencesOfString:@":" withString:@"%3A"]
        ...
        return [NSString stringWithFormat:@"magnet:?xt=urn:btih:%@&dn=%@%@", self.torrentHash, [self.torrentName orSome:@""],
                                          [trackers count] ? [NSString stringWithFormat:@"&%@", [trackers componentsJoinedByString:@"&"]] : @""];
    }
    return nil;
}

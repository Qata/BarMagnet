        ...
        if ([[[FileHandler.sharedInstance oldWebDataValueForKey:@"url"] orSome:@""] length]) {
            ...
            if ([url length]) {
                ...
                [NSUserDefaults.standardUserDefaults setObject:@[ @{
                                                         @"name" : @"Default",
                                                         @"url" : url,
                                                         @"type" : [FileHandler.sharedInstance settingsValueForKey:@"server_type"],
                                                         @"username" : username,
                                                         @"password" : password,
                                                         @"port" : port,
                                                         @"use_ssl" : useSSL,
...
- (BOOL)application:(UIApplication *)application openURL:(NSURL *)url sourceApplication:(NSString *)sourceApplication annotation:(id)annotation {
    if ([url isFileURL]) {
        [[TorrentDelegate sharedInstance] handleTorrentFile:url viewController:application.keyWindow.rootViewController];
    } else {
...
- (void)applicationWillResignActive:(UIApplication *)application {
    ...
    // phone call or SMS message) or when the user quits the application and it begins the transition to the background state. Use this method to pause ongoing
...
- (void)applicationDidEnterBackground:(UIApplication *)application {
    ...
    // to its current state in case it is terminated later. If your application supports background execution, this method is called instead of

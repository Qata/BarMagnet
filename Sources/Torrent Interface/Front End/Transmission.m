...
- (NSMutableURLRequest *)HTTPRequestWithMethod:(NSString *)method andHashes:(NSArray *)hashes {
    ...
    [request setHTTPBody:[NSJSONSerialization dataWithJSONObject:@{ @"method" : method,
...
- (NSMutableURLRequest *)checkTorrentJobs {
    ...
    NSString *localToken =
        [[[NSURLConnection sendSynchronousRequest:aRequest returningResponse:nil error:nil] toUTF8String] getStringBetween:@"X-Transmission-Session-Id: "
                                                                                                                 andString:@"</code>"];
    ...
    [request setHTTPBody:[NSJSONSerialization dataWithJSONObject:@{
                 ...
                 @"arguments" : @{
                     @"fields" : @[
                         @"hashString", @"name", @"percentDone", @"status", @"sizeWhenDone", @"downloadedEver", @"uploadedEver", @"peersGettingFromUs",
                         @"peersSendingToUs", @"rateDownload", @"rateUpload", @"eta", @"uploadRatio", @"addedDate", @"doneDate"
                     ]
                 }
             }
                                                         options:0
    ...
    for (NSDictionary *dict in [self getTorrentJobs]) {
        ...
        if (dict[@"hashString"]) {
            [self insertTorrentJobsDictWithArray:@[
                dict[@"hashString"], dict[@"name"], dict[@"percentDone"], status, [dict[@"rateDownload"] transferRateString], [dict[@"rateUpload"] transferRateString],
                ETA, [dict[@"downloadedEver"] sizeString], [dict[@"uploadedEver"] sizeString], dict[@"sizeWhenDone"], dict[@"peersGettingFromUs"],
                dict[@"peersSendingToUs"], dict[@"rateDownload"], dict[@"rateUpload"], dict[@"uploadRatio"], dict[@"addedDate"], dict[@"doneDate"]
            ]
                                        intoDict:tempJobs];
        }
    }
...
- (NSString *)commonAppendString {
    return
        [[[[FileHandler.sharedInstance webDataValueForKey:@"relative_path"] orSome:@""] stringByAppendingPathComponent:@"transmission"] stringWithPrecedingSlash];
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    if ([[responseData toUTF8String] rangeOfString:@"X-Transmission-Session-Id"].location != NSNotFound && storedRequest) {
        [storedRequest setValue:[responseData.toUTF8String getStringBetween:@"X-Transmission-Session-Id: " andString:@"</code>"]

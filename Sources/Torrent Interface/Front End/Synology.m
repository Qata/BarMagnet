...
- (NSURLRequest *)APIRequestTo:(NSString *)name
                    withMethod:(NSString *)method
             withParameterKeys:(NSArray *)parameterKeys
            andParameterValues:(NSArray *)parameterValues
    ...
    if (isUpload) {
        ...
        NSMutableString *URLString = [NSMutableString
            stringWithFormat:@"%@%@?api=%@&version=%@&method=%@", [self getAppendedURLWithoutAuth], APIDict[@"path"], APIDict[@"maxVersion"], name, method];
...
- (NSMutableURLRequest *)checkForSID:(NSMutableURLRequest *)request {
    if ([sid length]) {
        if ([[[request URL] absoluteString] rangeOfString:@"?"].location != NSNotFound) {
            ...
            [request setHTTPBody:[[[[request HTTPBody] toUTF8String] stringByAppendingString:[NSString stringWithFormat:@"&_sid=%@", sid]]
...
- (NSURLRequest *)loginRequest {
    ...
    return [NSURLRequest
        requestWithURL:[NSURL URLWithString:[NSString
                                                stringWithFormat:@"%@%@?api=%@&version=%@&method=login&account=%@&passwd=%@&session=DownloadStation&format=sid",
                                                                 [self getAppendedURLWithoutAuth], APIDict[@"path"], key, APIDict[@"maxVersion"],
                                                                 [[self getWebDataForKey:@"username"] orSome:@""],
                                                                 [[self getWebDataForKey:@"password"] orSome:@""]]]];
}
...
- (NSURLRequest *)logoutRequest {
    ...
    NSMutableURLRequest *request = [NSMutableURLRequest
        requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@?api=%@&version=%@&method=logout&session=DownloadStation&_sid=%@",
                                                                       [self getAppendedURLWithoutAuth], APIDict[@"path"], key, APIDict[@"maxVersion"], sid]]];
...
- (NSMutableURLRequest *)checkTorrentJobs {
    ...
    if ([sid length]) {
        return [self
            checkForSID:[NSMutableURLRequest
                            requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@?api=%@&version=%@&method=list&additional=detail,transfer,file",
                                                                                           [self getAppendedURLWithoutAuth], APIDict[@"path"], key,
                                                                                           APIDict[@"maxVersion"]]]]];
    }
    return nil;
}
...
- (NSDictionary *)virtualHandleTorrentJobs {
    ...
    for (NSDictionary *dict in [self getTorrentJobs]) {
        ...
        if ([dict[@"additional"][@"transfer"][@"speed_download"] integerValue] && ![percentDone isEqual:[[self class] completeNumber]]) {
            ETA = [@(([dict[@"size"] integerValue] - [dict[@"additional"][@"transfer"][@"size_downloaded"] integerValue]) /
                     [dict[@"additional"][@"transfer"][@"speed_download"] integerValue]) ETAString];
        }
        NSNumber *ratio = @0;
        ...
        [self insertTorrentJobsDictWithArray:@[
            dict[@"id"], dict[@"title"], percentDone, [dict[@"status"] sentenceParsedString],
            [dict[@"additional"][@"transfer"][@"speed_download"] transferRateString], [dict[@"additional"][@"transfer"][@"speed_upload"] transferRateString], ETA,
            [dict[@"additional"][@"transfer"][@"size_downloaded"] sizeString], [dict[@"additional"][@"transfer"][@"size_uploaded"] sizeString],
            [dict[@"size"] toNumber], dict[@"additional"][@"detail"][@"connected_leechers"], dict[@"additional"][@"detail"][@"connected_seeders"],
            dict[@"additional"][@"transfer"][@"speed_download"], dict[@"additional"][@"transfer"][@"speed_upload"], ratio
        ]
                                    intoDict:tempJobs];
    }
    return tempJobs;
...
- (NSURLRequest *)virtualHandleMagnetLink:(NSString *)magnetLink {
    ...
    NSMutableURLRequest *request =
        [NSMutableURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@", [self getAppendedURLWithoutAuth], APIDict[@"path"]]]];
    [request setHTTPMethod:@"POST"];
    [request setHTTPBody:[[NSString stringWithFormat:@"api=%@&version=%@&method=create&uri=%@&username=%@&password=%@", key, APIDict[@"maxVersion"],
                                                     [[magnetLink stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLHostAllowedCharacterSet]] encodeAmpersands],
                                                     [[self getWebDataForKey:@"username"] orSome:@""], [[self getWebDataForKey:@"password"] orSome:@""]]
                             dataUsingEncoding:NSUTF8StringEncoding]];
...
- (NSMutableURLRequest *)HTTPRequestWithMethod:(NSString *)method andHash:(NSString *)hash {
    NSString *key = @"SYNO.DownloadStation.Task";
    NSDictionary *APIDict = APIInfo[key];
    return [self checkForSID:[NSMutableURLRequest
                                 requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@?api=SYNO.DownloadStation.Task&version=%@&method=%@&id=%@",
                                                                                                [self getAppendedURLWithoutAuth], APIDict[@"path"],
                                                                                                APIDict[@"maxVersion"], method, hash.lowercaseString]]]];
}
...
- (NSURLRequest *)virtualRemoveTorrent:(NSString *)hash removeData:(BOOL)removeData {
    ...
    return [self
        checkForSID:[NSMutableURLRequest
                        requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@?api=%@&version=%@&method=%@&id=%@&force_complete=%@",
                                                                                       [self getAppendedURLWithoutAuth], APIDict[@"path"], key,
                                                                                       APIDict[@"maxVersion"], @"delete", [hash lowercaseString], @"false"]]]];
}

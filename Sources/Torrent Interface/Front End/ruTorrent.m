...
enum { PAUSING = 0, STARTED = 1, PAUSED = 1 << 1, CHECKING = 1 << 2, HASHING = 1 << 3, ERROR = 1 << 4 };
...
- (NSMutableURLRequest *)HTTPRequestWithMethod:(NSString *)method andHashes:(NSArray *)hashes {
  return [NSMutableURLRequest requestWithURL:[[NSURL URLWithString:self.getAppendedURL]
                                                 URLByAppendingPathComponent:[NSString stringWithFormat:@"php/rpc.php?action=%@&hashes=%@", method,
                                                                                                        [hashes componentsJoinedByString:@","]]]];
}

- (NSMutableURLRequest *)checkTorrentJobs {
  return [NSMutableURLRequest
      requestWithURL:[[NSURL URLWithString:self.getAppendedURL]
                         URLByAppendingPathComponent:[NSString stringWithFormat:@"php/rpc.php?action=list&args=%@", [@[
                                                                                  @"get_hash", @"get_state", @"get_name", @"get_down_rate", @"get_up_rate",
                                                                                  @"get_bytes_done", @"get_up_total", @"get_size_bytes", @"get_peers_accounted",
                                                                                  @"get_peers_complete", @"is_open", @"get_creation_date"
...
- (NSDictionary *)virtualHandleTorrentJobs {
  ...
  for (NSDictionary *dict in [self getTorrentJobs]) {
    ...
    [self insertTorrentJobsDictWithArray:@[
      dict[@"get_hash"], dict[@"get_name"], percentDone, status, [dict[@"get_down_rate"] transferRateString], [dict[@"get_up_rate"] transferRateString], ETA,
      [dict[@"get_bytes_done"] sizeString], [dict[@"get_up_total"] sizeString], [dict[@"get_size_bytes"] toNumber], dict[@"get_peers_accounted"],
      dict[@"get_peers_complete"], dict[@"get_down_rate"], dict[@"get_up_rate"],
      @([dict[@"get_bytes_done"] doubleValue] ? [dict[@"get_up_total"] doubleValue] / [dict[@"get_bytes_done"] doubleValue] : 0), dict[@"get_creation_date"]
    ]
                                intoDict:tempJobs];
  }
  return tempJobs;
...
- (NSMutableURLRequest *)universalPOSTSetting {
  ...
  [dir length] ? [requestAppend addObject:[NSString stringWithFormat:@"dir_edit=%@", [dir stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]]
               : nil;
  [label length] ? [requestAppend addObject:[NSString stringWithFormat:@"label=%@", [label stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]]
                 : nil;
...
- (NSURLRequest *)virtualHandleMagnetLink:(NSString *)magnetLink {
  ...
  [request setHTTPBody:[[@"url=" stringByAppendingString:[[magnetLink stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] encodeAmpersands]]

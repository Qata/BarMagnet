  ...
  for (NSArray *array in [self getTorrentJobs]) {
    ...
    if (array.count > 19) {
      ...
      [self insertTorrentJobsDictWithArray:@[
        array[0], array[2], array[4], status, [array[9] transferRateString], [array[8] transferRateString], [array[10] ETAString], [array[5] sizeString],
        [array[6] sizeString], array[3], array[12], array[14], array[9], array[8], @([array[7] integerValue] / 1000.), array[23], array[24]
      ]
                                  intoDict:tempJobs];
    } else {
      ...
      [self insertTorrentJobsDictWithArray:@[
        array[0], array[2], array[4], status, [array[9] transferRateString], [array[8] transferRateString], [array[10] ETAString], [array[5] sizeString],
        [array[6] sizeString], array[3], array[12], array[14], array[9], array[8], @([array[7] integerValue] / 1000.)
      ]
                                  intoDict:tempJobs];
    }
  }
...
- (NSURLRequest *)virtualHandleMagnetLink:(NSString *)magnetLink {
  ...
  storedURLString = [NSString stringWithFormat:@"%@?token=%%@&action=add-url&s=%@", [self getAppendedURL],
                                               [[magnetLink encodeAmpersands] stringByReplacingOccurrencesOfString:@"%" withString:@"%%"]];
  storedRequest = [NSMutableURLRequest new];
  return [self tokenRequest];
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
  if ([[responseData toUTF8String] rangeOfString:@"<div id='token' style='display:none;'>"].location != NSNotFound && storedRequest) {
    [storedRequest setURL:[NSURL URLWithString:[NSString stringWithFormat:storedURLString, [[responseData toUTF8String]
                                                                                               getStringBetween:@"<div id='token' style='display:none;'>"
                                                                                                      andString:@"</div>"]]]];

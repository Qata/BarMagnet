...
//  Bar Magnet
//
//  Created by Carlo Tortorella on 13/06/13.
...
- (NSDictionary *)virtualHandleTorrentJobs
{
	...
	for (NSDictionary * dict in torrentJobs)
	{
        ...
        {
			[self insertTorrentJobsDictWithArray:@[dict[@"hash"], dict[@"name"], dict[@"progress"], state, dict[@"dlspeed"], dict[@"upspeed"], dict[@"eta"]] intoDict:tempJobs];
        }
	}
	return tempJobs;
}
...
{
	return [ConnectionHandler createMultipartFormRequestWithHost:[[[self universalPOSTSetting] URL] URLByAppendingPathComponent:@"upload"] dispositionName:@"torrentfile" fileURL:fileURL andData:fileData];
}
